...
//Константа положения синхронизируемого файла

type Result<T> = std::result::Result<T, Box<dyn std::error::Error + Send + Sync + 'static>>;
//Специфический тип для распространения ошибок
type Recipes = Vec<Recipe>;

static KEYS: Lazy<identity::Keypair> = Lazy::new(|| identity::Keypair::generate_ed25519());
//Генератор ключей
static PEER_ID: Lazy<PeerId> = Lazy::new(|| PeerId::from(KEYS.public()));
//PeerID - персональный идентификатор узла генерируемый из ключа
static TOPIC: Lazy<Topic> = Lazy::new(|| Topic::new("recipes"));
//Топик - тема или множество, на которое может подписаться пир
...
//Структура рассылаемого обьекта
...
//Перечисление типа операции для получения данных с одного конкретного узла или со всез

#[derive(Debug, Serialize, Deserialize)]
struct ListRequest {
    mode: ListMode,
}
//Структура функции List для запроса данных
...
//Струтура функции List для ответа
